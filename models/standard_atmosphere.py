...
def stdatmo(h):
    ...
    data_from_file = {
        "h": data_from_file[:, 0] * ureg.m,  # altitude
        ...
        "kvisc": data_from_file[:, 5] * ureg.m ** 2 / ureg.s,
    }  # kinematic viscosity

    # these functions all use SI units
    interp_fcns = {
        "\rho": interp.interp1d(
            data_from_file["h"].magnitude,
            data_from_file["\rho"].magnitude,
            kind="cubic",
        ),
        "a": interp.interp1d(
            data_from_file["h"].magnitude, data_from_file["a"].magnitude, kind="cubic"
        ),
        "T": interp.interp1d(
            data_from_file["h"].magnitude, data_from_file["T"].magnitude, kind="cubic"
        ),
        "P": interp.interp1d(
            data_from_file["h"].magnitude, data_from_file["P"].magnitude, kind="cubic"
        ),
        "kvisc": interp.interp1d(
            data_from_file["h"].magnitude,
            data_from_file["kvisc"].magnitude,
            kind="cubic",
        ),
    }
    ...
    output = {
        "\rho": interp_fcns["\rho"](h_correctUnits) * data_from_file["\rho"].units,

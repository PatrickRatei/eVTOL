...
for config in cases:
    ...
    for i, case in enumerate(cases[config]):
        ...
        aircraft = aircraft.standard_substitutions(
            config=config, autonomousEnabled=c["autonomousEnabled"]
        )
        ...
        mission_cost = mission_cost.standard_substitutions(
            isRevenueMissionPiloted=c["isRevenueMissionPiloted"],
            isDeadheadMissionPiloted=c["isDeadheadMissionPiloted"],
        )
        ...
        print(
            case
            + "aircraft cost: %0.1f per kg"
            % solution("OnDemandAircraft.Airframe.cost_per_mass")
            .to(ureg.kg ** -1)
            .magnitude
        )
        ...
        T_perRotor = solution(
            "OnDemandRevenueMission.HoverTakeoff.OnDemandAircraftHoverPerformance.RotorsPerformance.T_perRotor"
        )
        T_A = solution(
            "OnDemandRevenueMission.HoverTakeoff.OnDemandAircraftHoverPerformance.RotorsPerformance.T/A"
        )
        V_tip = solution(
            "OnDemandRevenueMission.HoverTakeoff.OnDemandAircraftHoverPerformance.RotorsPerformance.v_{tip}"
        )
        Cl_mean = solution(
            "OnDemandRevenueMission.HoverTakeoff.OnDemandAircraftHoverPerformance.RotorsPerformance.Cl_{mean}"
        )
        rho = solution(
            "OnDemandRevenueMission.HoverTakeoff.HoverFlightState.FixedStandardAtmosphere.\\rho"
        )
...
for i, config in enumerate(cases):
    ...
    for j, case in enumerate(cases[config]):
        ...
        solution = cases[config][case]["solution"]
        cppk = (
            solution("OnDemandMissionCost.cost_per_passenger_km")
            .to(ureg.km ** -1)
            .magnitude
        )

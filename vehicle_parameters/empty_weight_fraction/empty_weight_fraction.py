...
for config in configs:
    ...
    configs[config]["cost_per_trip_per_passenger"] = np.zeros(
        np.size(weight_fraction_array)
    )
...
plt.grid()
plt.xlim(
    xmin=np.min(c["weight_fraction_array"]), xmax=np.max(c["weight_fraction_array"])
)
plt.ylim(ymin=0)

...
for config in configs:
    ...
    configs[config]["theta"]["rotational"]["f_fund"] = (
        np.zeros([np.size(B_array), np.size(theta_array)]) * ureg.turn / ureg.s
    )
    configs[config]["theta"]["rotational"]["SPL_A"] = np.zeros(
        [np.size(B_array), np.size(theta_array)]
    )
    ...
    for i, B in enumerate(B_array):
        for j, theta in enumerate(theta_array):
            ...
            f_peak, SPL, spectrum = rotational_noise(
                T_perRotor,
                Q_perRotor,
                R,
                VT,
...
for i, config in enumerate(configs):
    ...
    for j, B in enumerate(B_array):
        ...
        plt.plot(
            theta_array.to(ureg.degree).magnitude,
            SPL_rotational,
            color="black",
            linewidth=1.5,
            linestyle=style["linestyle"][j],
            marker=style["marker"][j],
            markersize=style["markersize"],
            fillstyle=style["fillstyle"][j],
...
title_str = (
    "Aircraft parameters: battery energy density = %0.0f Wh/kg; %s\n"
